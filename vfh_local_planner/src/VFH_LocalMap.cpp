...
namespace local_planner
{
    VFH_LocalMap::VFH_LocalMap(){
        ros::param::param("VFH_Local_Planner/CellSize", cellSize, 0.1);
        ros::param::param("VFH_Local_Planner/WindowSize", windowSize, 33);
        // cells = new double*[windowSize];
        // for (int i = 0; i < windowSize; i++)
        // {
        //     cells[i] = new double[windowSize];
        // }
        Clear();
    }
    ...
    void VFH_LocalMap::DrawLine (int x1, int y1, int x2, int y2, double value){
        ...
        for (curpixel = 0; curpixel <= numpixels; ++curpixel) {
            if (VFH_LocalMap::IsIn (x, y)){
                // ROS_INFO("windowSize = %d \t y = %3d \t x = %3d \t cells[y][x] = %f \t value = %f", windowSize, y, x, cells[y][x], value);
                cells[y][x] = value;
            }
            else
                break;
